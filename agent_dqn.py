...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.rand() < epsilon:
        # Choose a random action and object
        action_index = np.random.randint(NUM_ACTIONS)
        object_index = np.random.randint(NUM_OBJECTS)
    else:
        # Choose the action and object with the highest Q-value
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    maxq_next = 0.5 * (q_values_action_next.max().item() +
                       q_values_object_next.max().item())

    q_values_action, q_values_object = model(current_state_vector)

    q_value_cur_state_action = q_values_action[action_index]
    q_value_cur_state_object = q_values_object[object_index]
    ...
    loss_action = F.smooth_l1_loss(q_value_cur_state_action, torch.tensor(target))
    loss_object = F.smooth_l1_loss(q_value_cur_state_object, torch.tensor(target))

    loss = loss_action + loss_object
