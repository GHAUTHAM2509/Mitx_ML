...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    if not terminal:
        q_func[current_state_1, current_state_2, action_index, object_index] += ALPHA * (reward + GAMMA * np.max(q_func[next_state_1, next_state_2]) - q_func[current_state_1, current_state_2, action_index, object_index])
    else:
        q_func[current_state_1, current_state_2, action_index, object_index] += ALPHA * (reward - q_func[current_state_1, current_state_2, action_index, object_index])
...
def run_episode(for_training):
    ...
    Returns:
        float: cumulative discounted reward if testing, None if training
    """
    epsilon = TRAINING_EP if for_training else TESTING_EP

    epi_reward = 0.0  # cumulative discounted reward
    gamma_power = 1.0  # to keep track of GAMMA^t

    # Initialize the game for each episode
